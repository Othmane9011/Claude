...
/// ---------------------------------------------------------------------------
/// Providers utilisés: mêmes logiques que dans vet_details_screen.dart
/// ---------------------------------------------------------------------------
...
final _naiveSlotsProvider =
  ...
  String dayLabel(String yyyymmdd, int weekday1to7) {
    ...
    final mm = int.tryParse(yyyymmdd.substring(5,7)) ?? 1;
    final dd = int.tryParse(yyyymmdd.substring(8,10)) ?? 1;
    final j  = (weekday1to7 >= 1 && weekday1to7 <= 7) ? jours[weekday1to7 - 1] : '';
    final m  = (mm >= 1 && mm <= 12) ? mois[mm - 1] : '';
    return '$j $dd $m';
  }
...
/// ---------------------------------------------------------------------------
/// Ecran FLOW de réservation (réutilise la logique de VetDetails)
/// ---------------------------------------------------------------------------
class BookingFlowScreen extends ConsumerStatefulWidget {
  ...
  final String providerId;
  final String? serviceId; // 👈 permet une pré-sélection si connue
            ...
            children: [
              ...
              services.when(
                ...
                data: (list) {
                  ...
                  // init sélection (prend serviceId fourni si possible)
                  _selectedServiceId ??= (() {
                    final wanted = (widget.serviceId ?? '').toString();
                    if (wanted.isNotEmpty) {
                      final found = list.firstWhere(
                        (m) => (m['id'] ?? '').toString() == wanted,
                        orElse: () => const {},
                      );
                      if (found.isNotEmpty) return wanted;
                    }
