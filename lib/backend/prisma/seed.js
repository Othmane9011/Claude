...
async function main() {
  ...
  const provider = await prisma.providerProfile.upsert({
    where: { userId: proUser.id }, // userId est unique -> ok pour upsert
    update: {
  ...
  const day = (d, sH, sM, eH, eM) => ({
    providerId: provider.id,
    weekday: d,                     // 1=Mon..7=Sun
    startMin: sH * 60 + sM,
    endMin: eH * 60 + eM,
    // !! PAS de timezone ici: ta table ne l'a pas
  });
  ...
  await prisma.pet.upsert({
    where: { id: 'seed-pet-1' }, // id est unique -> ok
